...
export const sRgbCompanding = (value) => {
  return ((value <= SRGB_NORMALIZED_BELOW ? 12.92 * value : 1.055 * Math.pow(value, 0.416666) - 0.055) * 255);
};
/**
 * Gamma Companding
 * @param {number}                - color value
...
export const gammaCompanding = (value, gamma) => {
  return value < 0 ? -Math.pow(Math.abs(value), 1 / gamma) * 255 : Math.pow(value, 1 / gamma) * 255;
};
/**
 * L* Companding
 * @param {number}                - color value
