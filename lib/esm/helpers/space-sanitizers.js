 ...
 */
export const sanitizeColor = (space, color) => {
  ...
  switch (space) {
    ...
    case "rgba_m":
      ...
      if (space.includes("a")) {
        sanitized.alpha = clamp(sanitized.alpha ?? sanitized.a, 0, 1);
      }
      break;
    case "hsl":
    case "hsl_m":
      ...
      sanitized.lightness = sanitized.lightness != null
        ? clamp(sanitized.lightness, 0, 100)
        : sanitized.l ?? sanitized.v;
      if (space.includes("a")) {
        sanitized.alpha = clamp(sanitized.alpha ?? sanitized.a, 0, 1);
      }
      break;
    ...
    case "cmyk_m":
      ...
      if (space.includes("k")) {
        sanitized.key = clamp(sanitized.key ?? sanitized.k, 0, 100);
      }
      break;
    case "lab":
    case "lab_m":
